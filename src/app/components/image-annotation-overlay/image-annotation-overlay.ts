...
export class ImageAnnotationOverlayComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['imageUrl']) {
      ...
      queueMicrotask(() => this.ensureImageLoaded(0));
    }

  }

  private ensureImageLoaded(attempt: number): void {
    const img = this.imgEl?.nativeElement;
    if (!img) return;
    if (img.complete && img.naturalWidth > 0) {
      this.onImageLoad();
    ...
    requestAnimationFrame(() => this.ensureImageLoaded(attempt + 1));
  }
  ...
  private recomputeRenderedSize(): void {
    ...
    // Keep markers from obscuring text by lifting them above the word.

    // Use a size-relative offset so it behaves well across screen sizes.

    const markerPx = this.computeMarkerSizePx();

    this.markerYOffsetPx = Math.max(10, Math.round(markerPx * 0.85));
  ...
  getMarkerStyle(box: OverlayBox): Record<string, string> {
    ...
    const bg = (box.color || '#e53935').trim();
    const fg = this.markerTextColor(bg);
    const border = this.markerBorderColor(bg);
    ...
    return {
      ...
      '--marker-y-offset': `${this.markerYOffsetPx}px`,

      backgroundColor: bg,
      color: fg,
      borderColor: border,
      textShadow: fg === '#ffffff' ? '0 1px 2px rgba(0,0,0,0.35)' : '0 1px 0 rgba(255,255,255,0.45)',

    } as Record<string, string>;

  }

  private markerTextColor(color: string): string {
    const rgb = this.parseHexToRgb(color);
    if (!rgb) return '#ffffff';
    const { r, g, b } = rgb;

    const lum = (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255;
    return lum > 0.72 ? '#111827' : '#ffffff';
  }

  private markerBorderColor(color: string): string {
    const rgb = this.parseHexToRgb(color);
    if (!rgb) return 'rgba(255, 255, 255, 0.95)';
    const { r, g, b } = rgb;
    const lum = (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255;
    return lum > 0.72 ? 'rgba(17, 24, 39, 0.55)' : 'rgba(255, 255, 255, 0.95)';
  }

  private parseHexToRgb(color: string): { r: number; g: number; b: number } | null {
    const c = (color || '').trim();
  ...
  getUnderlineStyle(box: OverlayBox): Record<string, string> {
    ...
    if (!this.renderedWidth || !this.renderedHeight) {
    ...
    const wPx = (clamped.w / 100) * this.renderedWidth;

    const underlineHeightPx = Math.max(2, Math.min(4, Math.round(wPx * 0.06)));



    // Place underline at the bottom of the bbox, with a tiny gap so it reads like an underline.

    const gapPx = Math.max(1, Math.min(3, Math.round(underlineHeightPx * 0.75)));

    const yPx = ((clamped.y + clamped.h) / 100) * this.renderedHeight;

    const topPct = (Math.max(0, yPx + gapPx) / this.renderedHeight) * 100;
    ...
    return {
      ...
      '--underline-height': `${underlineHeightPx}px`
  ...
  private computeMarkerSizePx(): number {
    ...
    const isSmallScreen = this.renderedWidth < 420;

    const scaled = Math.round(this.renderedWidth * (isSmallScreen ? 0.045 : 0.024));

    const base = isSmallScreen


      ? Math.max(22, Math.min(32, scaled))


      : Math.max(18, Math.min(26, scaled));
