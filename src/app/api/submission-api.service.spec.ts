...
describe('SubmissionApiService canonical reads', () => {
  ...
  it('uses the same canonical assignment URL for desktop and mobile consumers', async () => {
    ...
    const desktopRequest = http.expectOne(`${environment.apiUrl}/api/submissions/assignment/assignment-1/my`);
    ...
    const mobileRequest = http.expectOne(`${environment.apiUrl}/api/submissions/assignment/assignment-1/my`);
  ...
  it('keeps result observation GET-only and retry explicitly POST-only', async () => {
    ...
    const readRequest = http.expectOne(`${environment.apiUrl}/api/submissions/assignment/assignment-1/my`);
    ...
    const retryRequest = http.expectOne(`${environment.apiUrl}/api/submissions/submission-1/ocr-corrections/regenerate`);
